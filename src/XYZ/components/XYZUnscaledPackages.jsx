...
import Scale from "../../Assets/RePackagesTemp.svg";
import ProfilePictureMini from "../../Assets/modules/ProfilePictureMini.jsx";

const fetchUnscaledPackages = () => {
  return new Promise((resolve) => {
    ...
    const unscaledPackages = [
      {
        id: "299999",
        packages: [
          { packageId: "9999", weight: "8.00", rescaled: "No" },
          { packageId: "10000", weight: "8.05", rescaled: "No" },
        ],
        weight: "16.05",
        date: "24/05/2024",
      },
      {
        id: "300000",
        packages: [
          { packageId: "10001", weight: "9.40", rescaled: "Yes" },
          { packageId: "10002", weight: "9.15", rescaled: "No" },
          { packageId: "10003", weight: "8.45", rescaled: "No" },
        ],
        weight: "27.00",
        date: "25/05/2024",
      },
      {
        id: "300001",
        packages: [
          { packageId: "10004", weight: "7.42", rescaled: "No" },
          { packageId: "10005", weight: "6.83", rescaled: "No" },
        ],
        weight: "14.25",
        date: "10/06/2024",
      },
      {
        id: "300002",
        packages: [{ packageId: "10006", weight: "12.00", rescaled: "No" }],
        weight: "12.00",
        date: formattedToday,
      },
    ];
    resolve(unscaledPackages);
  });
};

const UnscaledPackages = ({ onNavigate }) => {
  const [unscaledPackages, setUnscaledPackages] = useState([]);
  ...
  useEffect(() => {
    fetchUnscaledPackages().then((data) => setUnscaledPackages(data));
  }, []);
  ...
  const filterByTimespan = (unscaledPackages, timespan) => {
    const today = new Date();
    ...
    return unscaledPackages.filter((unscaledPackage) => {
      const packageDate = parseCustomDate(unscaledPackage.date);
  ...
  const filteredUnscaledPackages = filterByTimespan(
    unscaledPackages.filter((unscaledPackage) =>
      unscaledPackage.id.includes(searchTerm)
    ),
    selectedTimespan
  );

  const handleSort = (unscaledPackages, sortKey) => {
    ...
    setUnscaledPackages([...unscaledPackages].sort(sortFunction));
  };
  ...
  const moveToRescaled = (unscaledPackage) => {
    setUnscaledPackages((prevUnscaled) =>
      prevUnscaled.filter((pkg) => pkg.id !== unscaledPackage.id)
    );
    setRescaledPackages((prevRescaled) => [...prevRescaled, unscaledPackage]);
  };
  ...
  const renderMobileCards = () => {
    return filteredUnscaledPackages.map((unscaledPackage, index) => (
      <div className="card-wrapper" key={unscaledPackage.id}>
        <div className="card-header">
          <div>
            <div className="card-title">Shipment ID: </div>
            <div className={"card-subtitle"}>{unscaledPackage.id}</div>
          </div>
          {calculateRescaledStatus(unscaledPackage.packages) === "100.00" ? (
            moveToRescaled(unscaledPackage)
          ) : calculateRescaledStatus(unscaledPackage.packages) === "Error" ? (
            <button
              className="b-delete-button xyz"
              onClick={() => alert("NOTIFY CENTRA OR SMTHING")}
            >
              Alert
            </button>
          ) : (
            <button
              ...
              onClick={() => handleEdit(unscaledPackage)}
            >
              Details
            </button>
          )}
        ...
        <div className="card-content">
          <div className="card-description">
            ...
            <span className="card-temp">{unscaledPackage.packages.length}</span>
          </div>
          <div className="card-description">
            <span style={{ minWidth: "100px" }}>Weight</span>
            <span style={{ minWidth: "5px", padding: "0px 10px" }}>:</span>
            <span className="card-temp">{unscaledPackage.weight}</span>
          </div>
          <div className="card-description">
            <span style={{ minWidth: "100px" }}>Date</span>
            <span style={{ minWidth: "5px", padding: "0px 10px" }}>:</span>
            <span className="card-temp">{unscaledPackage.date}</span>
          </div>
          <div className="card-description">
            ...
            <span className="card-temp">
              {calculateRescaledStatus(unscaledPackage.packages) === "Error"
                ? "Error"
                : `${calculateRescaledStatus(unscaledPackage.packages)}%`}
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="p-container">
      ...
      <div className="top">
        ...
        <div className="a-title">Unscaled Packages</div>
      ...
      {windowWidth <= 560 ? (
        <div className="top-secondary">
          ...
          <div className="top-secondary-title"> Unscaled Packages </div>
        </div>
      ) : null}
                  ...
                  {selectedUnPackages.packages.map((unPackage, index) => (
                    <div className="card-wrapper" key={index}>
                      <div className="card-header">
                        ...
                        >
                          <img src={Scale} className="card-edit" />
                        </button>
                      </div>
                      <div className="card-content">
                        <div
                    ...
                    <table className="p-table-container">
                      <thead>
                        <tr>
                          <th
                            style={{ width: "15%" }}
                            onClick={() => handleSort(unscaledPackages, "id")}
                          >
                            ID {getArrow("id")}
                          </th>
                          <th
                            style={{ width: "22.5%" }}
                            onClick={() =>
                              handleSort(unscaledPackages, "packages")
                            }
                          >
                          ...
                          <th
                            ...
                            onClick={() =>
                              handleSort(unscaledPackages, "weight")
                            }
                          >
                          ...
                          <th
                            style={{ width: "25%" }}
                            onClick={() => handleSort(unscaledPackages, "date")}
                          >
                            Date {getArrow("date")}
                          </th>
                          <th
                            ...
                            onClick={() =>
                              handleSort(unscaledPackages, "rescaled")
                            }
                          >
                      ...
                      <tbody>
                        {filteredUnscaledPackages.map(
                          (unscaledPackage, index) => (
                            <tr key={index}>
                              <td>{unscaledPackage.id}</td>
                              <td>{unscaledPackage.packages.length}</td>
                              <td className="weight2-column">
                                {unscaledPackage.weight}
                              </td>
                              <td>{unscaledPackage.date}</td>
                              <td className="rescale-column">
                                {calculateRescaledStatus(
                                  unscaledPackage.packages
                                ) === "Error"
                                  ? "Error"
                                  : `${calculateRescaledStatus(
                                      unscaledPackage.packages
                              ...
                              <td>
                                {calculateRescaledStatus(
                                  ...
                                  <button
                                    className="b-delete-button xyz"
                                    onClick={() =>
                                      alert("NOTIFY CENTRA OR SMTHING")
                                    }
                                  >
                                    Alert
                                  </button>
...
export default UnscaledPackages;
