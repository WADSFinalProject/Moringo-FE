...
const SignUpPage = () => {
  ...
  const handlePhoneNumberChange = (event) => {
    ...
    const countryCodeLength = selectedCountry
      ? `+${selectedCountry.phone}`.length
      : 0;
    ...
    if (input.length >= countryCodeLength) {
      ...
      setPhoneNumber(`+${selectedCountry.phone}`);
    }
  };
  ...
  const handleSignUp = () => {
    ...
    if (!phoneNumber || phoneNumber === `+${selectedCountry.phone}`) {
      setPhoneError(true);
      phoneValid = false;
    } else {
      setPhoneError(false);
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  error={countryError}
                  helperText={countryError && "Country selection is required"}
                />
              )}
            />
            {selectedCountry && (
